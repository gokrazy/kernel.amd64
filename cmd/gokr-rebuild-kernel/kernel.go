...
func find(filename string) (string, error) {
	...
	path := filepath.Join(gopath, "src", "github.com", "gokrazy", "kernel.amd64", filename)
...
func main() {
	...
	cmd := exec.Command("go", "build", "-o", buildPath, "github.com/gokrazy/kernel.amd64/cmd/gokr-build-kernel")
	...
	dockerArgs = append(dockerArgs, "gokr-rebuild-kernel-amd64")
